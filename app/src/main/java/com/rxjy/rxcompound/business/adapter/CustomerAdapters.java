...
/**
 * Created by hjh on 2017/12/3.
 */
public class CustomerAdapters extends SingleBaseAdapter<CustomerBean.BodyBean, CustomerAdapters.ViewHolder> {


    public CustomerAdapters(Context context, List<CustomerBean.BodyBean> datas) {
        super(context, datas);
    }
    ...
    @Override
    public void onBindView(int position, final CustomerBean.BodyBean data, ViewHolder holder) {
        ...
        if (!StringUtils.isEmpty(data.getXingMing()) && !StringUtils.isEmpty(data.getGongSiMingCheng())) {
            ...
            holder.tvName.setText(data.getShouJiHaoYi());
        }
